import 'package:flutter/material.dart';
import 'package:flutter_screenutil/flutter_screenutil.dart';
import 'package:google_fonts/google_fonts.dart';

class AppTypography {
  // Light.
  static TextStyle kLight10 =
      GoogleFonts.plusJakartaSans(fontSize: 10.sp, fontWeight: FontWeight.w500);

  // Medium.
  static TextStyle kMedium48 =
      GoogleFonts.plusJakartaSans(fontSize: 48.sp, fontWeight: FontWeight.w400);
  static TextStyle kMedium40 =
      GoogleFonts.plusJakartaSans(fontSize: 40.sp, fontWeight: FontWeight.w400);
  static TextStyle kMedium32 =
      GoogleFonts.plusJakartaSans(fontSize: 32.sp, fontWeight: FontWeight.w400);
  static TextStyle kMedium24 =
      GoogleFonts.plusJakartaSans(fontSize: 24.sp, fontWeight: FontWeight.w400);
  static TextStyle kMedium20 =
      GoogleFonts.plusJakartaSans(fontSize: 20.sp, fontWeight: FontWeight.w400);
  static TextStyle kMedium18 =
      GoogleFonts.plusJakartaSans(fontSize: 18.sp, fontWeight: FontWeight.w400);
  static TextStyle kMedium16 =
      GoogleFonts.plusJakartaSans(fontSize: 16.sp, fontWeight: FontWeight.w400);
  static TextStyle kMedium14 =
      GoogleFonts.plusJakartaSans(fontSize: 14.sp, fontWeight: FontWeight.w400);
  static TextStyle kMedium12 =
      GoogleFonts.plusJakartaSans(fontSize: 12.sp, fontWeight: FontWeight.w400);

  // SemiBold.

  static TextStyle kSemiBold48 =
      GoogleFonts.plusJakartaSans(fontSize: 48.sp, fontWeight: FontWeight.w600);
  static TextStyle kSemiBold40 =
      GoogleFonts.plusJakartaSans(fontSize: 40.sp, fontWeight: FontWeight.w600);
  static TextStyle kSemiBold32 =
      GoogleFonts.plusJakartaSans(fontSize: 32.sp, fontWeight: FontWeight.w600);
  static TextStyle kSemiBold24 =
      GoogleFonts.plusJakartaSans(fontSize: 24.sp, fontWeight: FontWeight.w600);
  static TextStyle kSemiBold20 =
      GoogleFonts.plusJakartaSans(fontSize: 20.sp, fontWeight: FontWeight.w600);
  static TextStyle kSemiBold18 =
      GoogleFonts.plusJakartaSans(fontSize: 18.sp, fontWeight: FontWeight.w600);
  static TextStyle kSemiBold16 =
      GoogleFonts.plusJakartaSans(fontSize: 16.sp, fontWeight: FontWeight.w600);
  static TextStyle kSemiBold14 =
      GoogleFonts.plusJakartaSans(fontSize: 14.sp, fontWeight: FontWeight.w600);
  static TextStyle kSemiBold12 =
      GoogleFonts.plusJakartaSans(fontSize: 12.sp, fontWeight: FontWeight.w600);
  static TextStyle kSemiBold10 =
      GoogleFonts.plusJakartaSans(fontSize: 10.sp, fontWeight: FontWeight.w600);

  // Bold.

  static TextStyle kBold48 =
      GoogleFonts.plusJakartaSans(fontSize: 48.sp, fontWeight: FontWeight.w700);
  static TextStyle kBold40 =
      GoogleFonts.plusJakartaSans(fontSize: 40.sp, fontWeight: FontWeight.w700);
  static TextStyle kBold32 =
      GoogleFonts.plusJakartaSans(fontSize: 32.sp, fontWeight: FontWeight.w700);
  static TextStyle kBold24 =
      GoogleFonts.plusJakartaSans(fontSize: 24.sp, fontWeight: FontWeight.w700);
  static TextStyle kBold20 =
      GoogleFonts.plusJakartaSans(fontSize: 20.sp, fontWeight: FontWeight.w700);
  static TextStyle kBold18 =
      GoogleFonts.plusJakartaSans(fontSize: 18.sp, fontWeight: FontWeight.w700);

  // Extra Bold.

  static TextStyle kExtraBold40 =
      GoogleFonts.plusJakartaSans(fontSize: 40.sp, fontWeight: FontWeight.w800);
}
